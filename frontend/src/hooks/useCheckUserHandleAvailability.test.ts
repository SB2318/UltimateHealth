...
import {useCheckUserHandleAvailability} from './useCheckUserHandleAvailability';
...
describe('useCheckUserHandleAvailability', () => {
  ...
  it('executes mutation successfully and calls API', async () => {
    ...
    const {result} = renderHook(() => useCheckUserHandleAvailability('handle', true), {
      wrapper: makeWrapper(),
    });
