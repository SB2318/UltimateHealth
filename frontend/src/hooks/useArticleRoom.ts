...
export const useArticleRoom = (articleId: string | null = null, podcastId: string | null = null) => {
    ...
    useEffect(() => {
        ...
        // Join the room
        if (__DEV__) console.log(`Joining room: ${articleId ? 'article:' + articleId : 'podcast:' + podcastId}`);
        socket.emit('join-room', { articleId, podcastId });

        const handleRoomJoined = (data: any) => {
            if (__DEV__) console.log('✅ Joined room successfully:', data.room);
        };
        ...
        return () => {
            if (__DEV__) console.log(`Leaving room: ${articleId ? 'article:' + articleId : 'podcast:' + podcastId}`);
