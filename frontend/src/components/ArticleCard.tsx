...
const ArticleCard = ({
  ...
  const generatePDFData = async (title: string, htmlContent: string) => {
    try {
      ...
      logger.log('File flow reach upto now');
      const file = await generatePDF(options);

      await RNFS.moveFile(file.filePath, filePath);
      logger.log('File flow reach upto move');
      ...
      repost(Number(item._id), {
        onSuccess: data => {
          if (reposted === false) {
            ...
            logger.log('Repost success', data);
            setReposted(true);
            const body = {
              type: 'repost',
              userId: user_id,
              ...
              items={[
                ...
                },
                {
                  ...
                  action: () => {
                    ...
                    logger.log('modal visible', requestModalVisible);
                    // handleAnimation();
                  },
                  icon: 'edit',
                },
                  ...
                  if (isConnected) {
                    ...
                    likeMutation(undefined, {
                      onSuccess: (data: {
                        ...
                        logger.log('Article like success', data.likeStatus);
                  ...
                  if (isConnected) {
                    ...
                    saveMutation(undefined, {
                      onSuccess: async data => {
                        logger.log('Article save success', data);
                        Snackbar.show({
                          text: data.message,
                          duration: Snackbar.LENGTH_SHORT,
                        });
