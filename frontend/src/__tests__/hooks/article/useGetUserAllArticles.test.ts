...
<<<<<<< HEAD:frontend/src/hooks/useGetUserAllArticles.test.ts
import * as ReactRedux from '../../store/hooks';
import {useGetAllArticlesForUser} from './useGetUserAllArticles';
=======
import * as ReactRedux from 'react-redux';
import { useGetAllArticlesForUser } from '@/src/hooks/article/useGetUserAllArticles';

>>>>>>> upstream/main:frontend/src/__tests__/hooks/article/useGetUserAllArticles.test.ts
...
describe('useGetAllArticlesForUser', () => {
  ...
  it('executes mutation successfully and calls API', async () => {
    ...
    const {result} = renderHook(() => useGetAllArticlesForUser({ page: 1, selectedStatus: 1, visit: 1, setVisit: jest.fn(), setTotalPages: jest.fn(), setArticleData: jest.fn(), setPublishedLabel: jest.fn(), setProgressLabel: jest.fn(), setDiscardLabel: jest.fn() }), {
      wrapper: makeWrapper(),
    });
