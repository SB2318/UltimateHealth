...
import { useGetUserProfileImage } from '@/src/hooks/profile/useGetProfileImageById';
...
describe('useGetUserProfileImage', () => {
  ...
  it('executes mutation successfully and calls API', async () => {
    ...
    const {result} = renderHook(() => useGetUserProfileImage('test-id'), {
      wrapper: makeWrapper(),
    });
