...
<<<<<<< HEAD:frontend/src/hooks/useGetTotalWrites.test.ts
...
>>>>>>> upstream/main:frontend/src/__tests__/hooks/analytics/useGetTotalWrites.test.ts
...
describe('useGetTotalWrites', () => {
  ...
  it('executes mutation successfully and calls API', async () => {
    ...
    const {result} = renderHook(() => useGetTotalWrites({ user_id: '1', isConnected: true }), {
      wrapper: makeWrapper(),
    });
