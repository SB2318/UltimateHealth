...
import { useUpdatePodcastViewcount } from '@/src/hooks/podcast/useUpdatePodcastViewcount';
...
describe('useUpdatePodcastViewcount', () => {
  ...
  it('executes mutation successfully and calls API', async () => {
    ...
    const {result} = renderHook(() => useUpdatePodcastViewcount(), {
      wrapper: makeWrapper(),
    });
